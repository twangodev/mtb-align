...
otherwise needs them registered already. Comparison is an XOR down a pyramid, a
pixel of search per level.

No rotation, and no median worth thresholding at once a frame is half clipped.

```sh
cargo run --example align       # aligns a synthetic bracket into target/
```
...
sequences that do need rotation. OpenCV implements it as `findTransformECC`.

OpenCV's `AlignMTB`, whose conventions this follows and whose output the test
